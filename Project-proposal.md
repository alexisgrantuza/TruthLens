...
**Tagline:** *The Immutable Eye – Verifying Reality in the Age of AI.*
...
We live in a "post-truth" era.
  ...
  * **Erosion of Trust:** Journalists, insurance adjusters, and human rights activists have no easy way to prove that a photo *has not* been doctored since the moment it was taken.

### 2\. The Solution (The "Magic")

**TruthLens** is a mobile dApp (Decentralized App) that acts as a digital notary.

1.  **Capture:** Users take a photo *inside* the app.
...
5.  **Result:** A permanent, unchangeable record that *this* exact image existed at *this* exact time and place.

-----

### 3\. The 24-Hour Hackathon MVP Scope

*Do not try to build the next Instagram. Focus on the "Verification" workflow.*
...
3.  **Processing (The "Wow" Moment):**
      * Show a loading bar: *"Hashing Image..."* -\> *"Analyzing with AI..."* -\> *"Minting Proof to Blockchain..."*
4.  **Success Screen:**
      * Show the Photo.
...
| :--- | :--- | :--- |
| **Frontend** | **Next.js (React)** | Fast setup, easy PWA (Progressive Web App) for mobile camera access. |
| **Blockchain** | **Solana (Devnet)** | Extremely fast (sub-second finality), very cheap, and great tooling (Solana Web3.js). *Alternative: Polygon.* |
| **Storage** | **IPFS (via Pinata)** | You can't store images on-chain (too expensive). Store the *image* on IPFS, store the *hash/link* on-chain. |
...
| **Deepfake/AI Detection** | **Hive AI / Sensity** | *Optional:* If you have time, hit a specialized API to get a "Fake Score." |
...
#### **Phase 2: The Core Logic (Hours 4-12)**
  ...
  * **The Smart Contract (The Easy Way):**
      * *Don't write a complex Rust/Solidity contract.*
      ...
      * *Result:* You now have an on-chain record without writing a custom smart contract\!
...
#### **Phase 3: The AI Agent (Hours 12-18)**
  ...
  * **Prompt:** *"Analyze this image. 1. Describe the scene briefly. 2. Look for visual anomalies that suggest manipulation (blurring, lighting mismatches). Return JSON."*
  * **Display:** Show this AI "Opinion" next to the Blockchain "Fact."

#### **Phase 4: Polish & Pitch (Hours 18-24)**

  * **The "Verify" Page:** Create a page where anyone can upload *that same photo* later. The app hashes it again. If the new hash matches the on-chain hash, it turns **GREEN**. If one pixel changed, the hashes won't match, and it turns **RED**.
...
async function saveToChain(ipfsHash, imageHash) {
  const connection = new Connection("https://api.devnet.solana.com");
  const wallet = window.solana; // Provided by Phantom Wallet

  const message = JSON.stringify({ ipfs: ipfsHash, hash: imageHash });
  ...
  console.log("Success! View on Solscan:", signature);
}
```